...
import { exitFail } from '../utils/process-control';
import { getCredentials } from './git-credentials';
import { getDocsUrl } from './wiki-platforms';
...
const getFetchOptions = async (configGetter, gitToken, authMethod) => {
    ...
    const credentialsProvider = authMethodsToCredentialProvider[authMethod] || authMethodsToCredentialProvider.https;
...
const isPrivateRepo = async path => {
    try {
        const {status} = await axios.get(`${path.replace('https', 'http')}/info/refs?service=git-upload-pack`);
